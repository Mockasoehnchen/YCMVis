...
import CDC_core, GEX_APC, GEX_GRN,GEX_TRL, MET_CCM, MET_CWS,MET_DNA, TRP_ION,TRP_NUT,VOL_core
...
def data_to_graph():
    ...
    counter = 0
    modulList = [CDC_core.CDC_core(),GEX_APC.GEX_APC(),GEX_GRN.GEX_GRN(),GEX_TRL.GEX_TRL(),MET_CCM.MET_CCM(),MET_DNA.MET_DNA(),MET_CWS.MET_CWS(),TRP_ION.TRP_ION(),TRP_NUT.TRP_NUT()]
    #,VOL_core.VOL_core()] #Causes error?
    for model in modulList:
        ...
        for reaction in modeldict['reactions']:
            ...
            for source in modeldict['reactions'][reaction]['modifiers']:
                ...
                new_node['num_link'] += 1
                nodes[int(nodes_to_int_ids[source + modeldict['name']])]['num_link'] += 1
            if(len(new_node['compartments']) == 1):
                compartments[new_node['compartments'][0]]['species'].append(new_node['name'])
            else:
                #print("Reaction in multiple Compartments: "+ " ".join(new_node['compartments']))
                compartments[new_node['compartments'][0]]['species'].append(new_node['name'])
                compartments[new_node['compartments'][1]]['species'].append(new_node['name'])
            nodes.append(new_node)
            counter += 1
...
def use_dot():
    ...
    for node in nodes:
        text = text + node['name']+';'+os.linesep # not needed for dot beacuse all species are in a compartment but provides order used later
